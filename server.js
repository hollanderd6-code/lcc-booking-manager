...
// ============================================
// ROUTES API - GESTION DES FACTURES
// ============================================
...
app.get('/api/invoices', async (req, res) => {
  try {
    ...
    if (year) {
      paramCount++;
      query += ` AND EXTRACT(YEAR FROM issue_date) = $${paramCount}`;
      params.push(year);
    }

    query += ` ORDER BY issue_date DESC, id DESC LIMIT $${paramCount + 1} OFFSET $${paramCount + 2}`;
    params.push(limit, offset);

    const result = await pool.query(query, params);

    // Compter le total pour la pagination
    const countQuery = `SELECT COUNT(*) FROM invoices WHERE user_id = $1` + 
      (status ? ` AND status = $2` : '') +
      (year ? ` AND EXTRACT(YEAR FROM issue_date) = $${status ? 3 : 2}` : '');
...
app.get('/api/invoices/:id', async (req, res) => {
  try {
    ...
    const itemsResult = await pool.query(
      'SELECT * FROM invoice_items WHERE invoice_id = $1 ORDER BY order_index, id',
      [id]
    );
...
app.post('/api/invoices', async (req, res) => {
  ...
  try {
    ...
    if (!clientName || !items || items.length === 0) {
      ...
      return res.status(400).json({ error: 'Nom du client et au moins une ligne de facturation requis' });
    }

    // Générer le numéro de facture
    const invoiceNumberResult = await client.query(
      "SELECT get_next_invoice_number($1, 'FACT') as invoice_number",
      [user.id]
    );
    ...
    const calculatedDueDate = dueDate || 
      (issueDate ? new Date(new Date(issueDate).getTime() + 30 * 24 * 60 * 60 * 1000) : 
       new Date(Date.now() + 30 * 24 * 60 * 60 * 1000));

    // Insérer la facture
    const invoiceResult = await client.query(`
      INSERT INTO invoices (
        invoice_number, user_id, issue_date, due_date,
        client_name, client_email, client_phone, client_address,
        client_postal_code, client_city, client_country, client_siret,
        property_id, property_name, checkin_date, checkout_date, nights, guests,
        currency, vat_applicable, vat_rate,
        subtotal_ht, vat_amount, total_ttc,
        status, payment_method, notes, footer_text
      ) VALUES (
    ...
    `, [
      invoiceNumber, user.id, issueDate || new Date(), calculatedDueDate,
      clientName, clientEmail, clientPhone, clientAddress,
      clientPostalCode, clientCity, clientCountry || 'France', clientSiret,
      propertyId, propertyName, checkinDate, checkoutDate, nights, guests,
      currency || 'EUR', vatApplicable || false, vatRate || 0,
      subtotalHt, vatAmount, totalTtc,
      status, paymentMethod, notes, footerText
    ]);

    const invoiceId = invoiceResult.rows[0].id;

    // Insérer les lignes de facturation
    for (let i = 0; i < items.length; i++) {
      const item = items[i];
      const itemTotal = parseFloat(item.quantity) * parseFloat(item.unitPrice);
      
      await client.query(`
        INSERT INTO invoice_items (invoice_id, description, quantity, unit_price, total, order_index)
        VALUES ($1, $2, $3, $4, $5, $6)
      `, [invoiceId, item.description, item.quantity, item.unitPrice, itemTotal, i]);
    }
...
app.put('/api/invoices/:id', async (req, res) => {
  ...
  try {
    ...
    // On ne peut modifier que les factures en brouillon ou émises (pas les payées/annulées)
    if (['paid', 'cancelled'].includes(checkResult.rows[0].status)) {
      return res.status(400).json({ 
    ...
    const {
      clientName, clientEmail, clientPhone, clientAddress,
      clientPostalCode, clientCity, clientCountry, clientSiret,
      propertyName, checkinDate, checkoutDate, nights, guests,
      currency, vatApplicable, vatRate, items,
      paymentMethod, notes, footerText, issueDate, dueDate, status
    } = req.body;
    ...
    await client.query(`
      UPDATE invoices SET
        ...
        notes = $21,
        footer_text = $22,
        issue_date = COALESCE($23, issue_date),
        due_date = $24,
    ...
    `, [
      ...
      clientPostalCode, clientCity, clientCountry, clientSiret,
      propertyName, checkinDate, checkoutDate, nights, guests,
      currency, vatApplicable, vatRate,
      subtotalHt, vatAmount, totalTtc,
      paymentMethod, notes, footerText, issueDate, dueDate, status,
      id, user.id
    ]);

    // Supprimer les anciennes lignes et en créer de nouvelles si items fourni
    if (items && items.length > 0) {
      ...
      for (let i = 0; i < items.length; i++) {
        ...
        await client.query(`
          INSERT INTO invoice_items (invoice_id, description, quantity, unit_price, total, order_index)
          VALUES ($1, $2, $3, $4, $5, $6)
        `, [id, item.description, item.quantity, item.unitPrice, itemTotal, i]);
      }
    }
...
app.post('/api/invoices/:id/mark-paid', async (req, res) => {
  try {
    ...
    `, [paymentDate || new Date(), paymentMethod, id, user.id]);
    ...
    res.json({
      success: true,
      message: `Facture ${result.rows[0].invoice_number} marquée comme payée`
    });
...
app.post('/api/invoices/:id/cancel', async (req, res) => {
  try {
    ...
    const result = await pool.query(`
      ...
      WHERE id = $1 AND user_id = $2 AND paid = false
      RETURNING invoice_number
    `, [id, user.id]);

    if (result.rows.length === 0) {
      return res.status(404).json({ 
        error: 'Facture non trouvée ou déjà payée (impossible d\'annuler)' 
      });
    }

    res.json({
      success: true,
      message: `Facture ${result.rows[0].invoice_number} annulée`
    });
...
// ============================================
// 7. SUPPRIMER UNE FACTURE (brouillon seulement)
// ============================================
...
app.delete('/api/invoices/:id', async (req, res) => {
  try {
    ...
    if (result.rows.length === 0) {
      return res.status(404).json({ 
        error: 'Facture non trouvée ou impossible à supprimer (seuls les brouillons peuvent être supprimés)' 
      });
    }

    res.json({
      success: true,
      message: `Facture ${result.rows[0].invoice_number} supprimée`
    });
...
app.post('/api/invoices/upload-logo', upload.single('logo'), async (req, res) => {
  try {
    const user = await getUserFromRequest(req);
    if (!user) {
      return res.status(401).json({ error: 'Non autorisé' });
    ...
    // Sauvegarder dans user_settings_invoice
    await pool.query(`
      INSERT INTO user_settings_invoice (user_id, logo_url)
      VALUES ($1, $2)
      ON CONFLICT (user_id) 
      DO UPDATE SET logo_url = $2
    `, [user.id, logoUrl]);
...
// ============================================
// 9. PARAMÈTRES DE FACTURATION
// ============================================

app.get('/api/invoices/settings', async (req, res) => {
  try {
    const user = await getUserFromRequest(req);
    if (!user) {
      return res.status(401).json({ error: 'Non autorisé' });
    ...
    const result = await pool.query(
      'SELECT * FROM user_settings_invoice WHERE user_id = $1',
      [user.id]
    );
    ...
    const settings = result.rows[0] || {
      logo_url: null,
      default_vat_rate: 0,
      default_vat_applicable: false,
      default_currency: 'EUR',
      default_payment_terms: 30,
      default_footer_text: null,
      default_legal_mentions: null
    };
...
app.post('/api/invoices/settings', async (req, res) => {
  try {
    const user = await getUserFromRequest(req);
    if (!user) {
      return res.status(401).json({ error: 'Non autorisé' });
    ...
    await pool.query(`
      INSERT INTO user_settings_invoice (
        user_id, default_vat_rate, default_vat_applicable,
        default_currency, default_payment_terms,
        default_footer_text, default_legal_mentions
      )
      VALUES ($1, $2, $3, $4, $5, $6, $7)
      ON CONFLICT (user_id)
      DO UPDATE SET
        default_vat_rate = $2,
        default_vat_applicable = $3,
        default_currency = $4,
        default_payment_terms = $5,
        default_footer_text = $6,
        default_legal_mentions = $7
    `, [
      user.id, defaultVatRate, defaultVatApplicable,
      defaultCurrency, defaultPaymentTerms,
      defaultFooterText, defaultLegalMentions
    ]);
  ...
  try {
    ...
    const stats = await pool.query(`
      SELECT
        ...
        COALESCE(SUM(total_ttc) FILTER (WHERE status IN ('issued', 'overdue')), 0) as total_pending
      FROM invoices
