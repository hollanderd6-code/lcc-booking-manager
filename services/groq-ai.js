...
function buildSystemPrompt(ctx, temporalCtx, fewShotExamples) {
  ...
  const accessInfo = [];
  // Ne PAS inclure les codes d'accès si l'arrivée est dans plus d'1 jour
  const canShareCodes = temporalCtx && (temporalCtx.phase === 'during' || temporalCtx.isCheckinDay || (temporalCtx.phase === 'before' && temporalCtx.daysUntilCheckin <= 1));
  if (canShareCodes) {
    if (ctx.accessCode)         accessInfo.push(`- Code d'accès / boîte à clés : ${ctx.accessCode}`);
    if (ctx.wifiName)           accessInfo.push(`- Réseau WiFi : ${ctx.wifiName}`);
    if (ctx.wifiPassword)       accessInfo.push(`- Mot de passe WiFi : ${ctx.wifiPassword}`);
  } else {
    accessInfo.push(`- Codes d'accès / WiFi : NON DISPONIBLES — seront envoyés automatiquement le matin de l'arrivée.`);
  }
...
CODES D'ACCÈS / WiFi
• RÈGLE PRIORITAIRE (s'applique à TOUS les canaux, y compris Airbnb) : si l'arrivée est dans plus d'1 jour → NE JAMAIS donner les codes/accès/wifi. Répondre : "Les informations d'accès vous seront envoyées le matin de votre arrivée. C'est une arrivée autonome, vous recevrez un livret d'accueil détaillé avec toutes les instructions."
