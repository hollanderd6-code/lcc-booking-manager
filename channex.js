    ...
    if (!channex_room_type_id) {
      ...
      if (rts.length > 0) {
        ...
        if (clash.rows.length > 0) {
          const msg = `channex_room_type_id ${candidateId} est déjà mappé sur la propriété BH ${clash.rows[0].id} — connexion refusée pour éviter un room type non rattaché aux OTA`;
          console.error(`❌ [CHANNEX] ${msg}`);
          throw Object.assign(new Error(msg), { code: 'ROOM_TYPE_CLASH', clashPropertyId: clash.rows[0].id, roomTypeId: candidateId });
        } else {
...
async function processChannexBooking(pool, bookingData) {
  try {
    ...
    // Booking.com envoie les frais de ménage dans room.taxes avec type "Service Charge"
    const cleaning_from_taxes = taxes.find(t =>
      /clean|ménage|menage|cleaning|nettoyage|service.?charge/i.test(t.name || '') ||
      /service.?charge/i.test(t.type || '')
    );

    const amount_cleaning = cleaning_service
      ? parseFloat(cleaning_service.total_price || 0)
      : cleaning_from_taxes
    ...
    const pure_taxes = taxes.filter(t =>
      !/service.?charge/i.test(t.type || '') &&
      !/clean|ménage|menage|cleaning|nettoyage/i.test(t.name || '')
    );
    ...
    if (existing.rows.length === 0 && reservationStart && reservationEnd && channex_property_id) {
      ...
      if (dupCheck.rows.length > 1) {
        // Ambiguïté : plusieurs candidats → ne modifier aucun, signaler au webhook handler
