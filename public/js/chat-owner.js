...
async function openChat(conversationId) {
  ...
  // ✅ Quand on ouvre une conversation côté propriétaire, on marque toutes les notifications
  // de cette conversation comme lues (sinon le compteur "Messages non lus" reste bloqué).
  try {
    await markConversationRead(conversationId);
  } catch (e) {
    console.warn('⚠️ Impossible de mark-read:', e);
  }
  ...
  // Mettre à jour les compteurs (liste + sidebar)
  loadConversations();
...
async function markConversationRead(conversationId) {
  const token = localStorage.getItem('lcc_token');
  const resp = await fetch(`${API_URL}/api/chat/conversations/${conversationId}/mark-read`, {
    method: 'POST',
    headers: {
      'Authorization': 'Bearer ' + token
    }
