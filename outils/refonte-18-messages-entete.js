#!/usr/bin/env node
/* ============================================================
   outils/refonte-18-messages-entete.js
   Lot 18 : l'en-tete, le defilement, les onglets
   ============================================================

   ── 1. L'EN-TETE COMME AILLEURS ──────────────────────────────────
   « 9 non lus » en surtitre, « Messages » en grand, la loupe et vos
   initiales a droite. La meme forme que Calendrier et Aujourd'hui, avec
   le meme code : le rond #bhAvatarHeader est DEPLACE, pas recree — il
   n'en existe qu'un dans la page, et le dupliquer creerait deux etats a
   maintenir.

   Le surtitre se reecrit a chaque passage. Il ne se fige pas sur une
   valeur lue trop tot — la lecon du sous-titre du calendrier.

   ── 2. LA LISTE NE DEFILAIT PAS ──────────────────────────────────
   La colonne des conversations est un conteneur a hauteur fixe, prevu
   pour un ecran large ou la liste et la conversation coexistent. Sur
   telephone la liste depasse et rien ne bouge.

   Le module remonte les ancetres de la liste et assouplit ceux qui
   bloquent : hauteur figee, overflow cache. Il note lesquels dans
   bhVerifEnteteMsg().defilement — donc si le defilement casse ailleurs,
   on sait exactement ce qui a ete touche, et bhAnnulerEnteteMsg() le
   remet.

   Je n'impose pas une hauteur : je retire celles qui empechent. Fixer
   une hauteur en dur casserait la version large.

   ── 3. LES ONGLETS PARTENT ───────────────────────────────────────
   Templates, Statut, SMS, Statut SMS quittent la barre du haut. Vous
   les remettrez dans le menu — ils ne sont pas supprimes, seulement
   masques, et l'annulation les rend.

   La barre est reperee par son CONTENU : un conteneur qui porte a la
   fois « Templates » et « Statut ». Si les intitules changent, rien
   n'est masque et le diagnostic le dit — plutot que de masquer une
   barre au hasard.

   Usage :
     node outils/refonte-18-messages-entete.js --essai
     node outils/refonte-18-messages-entete.js
   ============================================================ */
...
const MODULE = path.join(PUBLIC, 'js', 'bh-entete-messages.js');
...
if (!fs.existsSync(PAGE)) echec('public/messages.html introuvable. Lancez depuis la racine du projet.');
let html = fs.readFileSync(PAGE, 'utf8');
if (html.indexOf('bh-messages-liste.js') === -1) echec('Le lot 17 n\'est pas applique.');
...
[
  ['l en-tete', 'function poserEntete()'],
  ['le defilement', 'function libererDefilement()'],
  ['les onglets', 'function barreOnglets()'],
  ['l annulation', 'bhAnnulerEnteteMsg'],
  ['le diagnostic', 'bhVerifEnteteMsg'],
].forEach(function (c) {
  if (SOURCE.indexOf(c[1]) === -1) echec('Verification : ' + c[0] + ' est absent du module.');
});

const BALISE = '<script src="js/bh-entete-messages.js"></script>';
let etatPage;

if (html.indexOf('bh-entete-messages.js') !== -1) {
  etatPage = 'balise deja presente';
} else {
  const ancre = html.indexOf('bh-messages-liste.js');
  const fin = html.indexOf('</script>', ancre);
  if (fin === -1) echec('Balise du lot 17 mal formee dans messages.html.');
  const pos = fin + '</script>'.length;
  html = html.slice(0, pos) + '\n' + BALISE + html.slice(pos);
  etatPage = 'balise ajoutee apres bh-messages-liste.js';
}
...
if (!ESSAI) {
  ...
  if (relu.indexOf('bhAnnulerEnteteMsg') === -1) echec("Le module n'est pas complet apres ecriture.");
  try { new Function(relu); } catch (e) { echec('Module invalide apres ecriture — ' + e.message); }
}

console.log('\n' + (ESSAI ? '— ESSAI, aucune ecriture —' : '— APPLIQUE ET VERIFIE —'));
console.log('  public/js/bh-entete-messages.js  (' + Math.round(SOURCE.length / 1024) + ' Ko)');
console.log('  messages.html                    ' + etatPage);
console.log('\n  1. En-tete a la forme des autres ecrans. Le rond est DEPLACE,');
console.log('     pas recree : il n\'en existe qu\'un dans la page.');
console.log('  2. Defilement : les ancetres a hauteur figee sont assouplis,');
console.log('     pas remplaces par une hauteur en dur — cela casserait la');
console.log('     version large. bhVerifEnteteMsg().defilement les nomme.');
console.log('  3. Templates, Statut, SMS masques. La barre est reperee par');
console.log('     son contenu ; si les intitules changent, rien n\'est masque');
console.log('     et le diagnostic le dit.');
console.log('\n  A verifier, cache vide : /messages.html');
console.log('    l\'en-tete « N non lus / Messages » avec loupe et initiales');
console.log('    la liste defile au pouce jusqu\'au bandeau IA');
console.log('    plus d\'onglets Templates / Statut / SMS');
