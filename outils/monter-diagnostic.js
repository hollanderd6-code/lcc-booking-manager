#!/usr/bin/env node
/* ============================================================
   Monter la route de diagnostic dans server.js
   ============================================================
...
const ROUTE = path.join(RACINE, 'routes', 'diagnostic-ota-routes.js');
...
if (!fs.existsSync(ROUTE)) {
  console.error('\n  \u2717 routes/diagnostic-ota-routes.js introuvable.');
  console.error('    Copiez-le d\'abord :  cp /tmp/reg/.../diagnostic-ota-routes.js routes/\n');
  process.exit(1);
}

let src = fs.readFileSync(CIBLE, 'utf8');

if (src.indexOf('diagnostic-ota-routes') !== -1) {
...
if (!nomApp || !nomPool || !nomAuth) {
  ...
  console.error('      require(\'./routes/diagnostic-ota-routes\')(app, pool, VOTRE_MIDDLEWARE);\n');
  process.exit(1);
}
...
const ligne =
  ...
  indent + '// Lecture seule : montre ce que le partenaire sait du logement.\n' +
  indent + 'require(\'./routes/diagnostic-ota-routes\')(' + nomApp + ', ' + nomPool + ', ' + nomAuth + ');\n\n';
...
console.log('    require(\'./routes/diagnostic-ota-routes\')(' + nomApp + ', ' + nomPool + ', ' + nomAuth + ');');
