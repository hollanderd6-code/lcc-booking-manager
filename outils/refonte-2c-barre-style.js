#!/usr/bin/env node
/* ============================================================
   outils/refonte-2c-barre-style.js
   Lot 2c : les icones de la maquette, et la barre en ilot
   ============================================================

   ── DEUX DEFAUTS VISIBLES ────────────────────────────────────────
   1. « Logements » a garde le balai du menage, et « Argent » les trois
      points de « Plus ». Le nom a change, le dessin non — c'est pire
      qu'avant : un balai qui mene aux logements ment activement.

   2. La barre touche les deux bords et le bas de l'ecran. Sur un
      telephone a coins arrondis, ses angles se font rogner par la dalle,
      et la capsule active se retrouve coupee a gauche — visible sur
      votre capture, « Aujourd'hui » est tronque.

   ── CE QUE FAIT LE LOT ───────────────────────────────────────────
   Les cinq icones de la maquette, dessinees au trait, a la place des
   anciennes :
       ...
       Argent        un portefeuille

   Et la barre devient un ilot : detachee des bords, arrondie a 26 px,
   posee sur un fond translucide. Les angles ne peuvent plus etre
   rognes, et la capsule a la place de respirer.

   ── LA CAPSULE ───────────────────────────────────────────────────
   .lg-capsule se place en mesurant le bouton actif. Deplacer la barre
   change ces mesures : le module declenche donc un evenement resize
   apres restylage, pour que la capsule se recalcule au lieu de rester
   ou elle etait. Sans cela, elle s'allumerait a cote — le defaut que
   vous m'avez signale au premier lot.

   Le bouton « + » est remonte au-dessus de l'ilot : il le chevauchait.

   Usage :
     node outils/refonte-2c-barre-style.js --essai
     node outils/refonte-2c-barre-style.js
   ============================================================ */
...
const MODULE = path.join(PUBLIC, 'js', 'bh-barre-style.js');
...
if (!fs.existsSync(path.join(PUBLIC, 'js', 'bh-barre-onglets.js'))) {
  echec('bh-barre-onglets.js absent. Lancez d\'abord refonte-2-barre-argent.js puis refonte-2b-barre-ids.js.');
}
...
const BALISE = '<script src="js/bh-barre-style.js"></script>';
const rapport = [];
...
PAGES.forEach(function (nom) {
  ...
  if (html.indexOf('bh-barre-style.js') !== -1) { rapport.push([nom, 'deja']); return; }

  const ancre = html.indexOf('bh-barre-onglets.js');
  if (ancre === -1) { rapport.push([nom, 'sans bh-barre-onglets — ignoree']); return; }
  ...
  rapport.push([nom, 'apres bh-barre-onglets.js']);
...
if (!ESSAI) {
  ...
  if (relu.indexOf('bhVerifStyle') === -1) echec("Le module n'est pas complet apres ecriture.");
  ['accueil:', 'calendrier:', 'messages:', 'logements:', 'argent:'].forEach(function (c) {
    if (relu.indexOf(c) === -1) echec('Le dessin ' + c + ' manque.');
  });
}
...
console.log('  public/js/bh-barre-style.js  (' + Math.round(SOURCE.length / 1024) + ' Ko)');
rapport.forEach(function (r) { console.log('  ' + (r[0] + '                    ').slice(0, 20) + r[1]); });
console.log('\n  maison · calendrier · bulle · immeuble · portefeuille');
console.log('  Barre en ilot : 12 px des bords, arrondie a 26 px, fond translucide.');
console.log('  Le « + » remonte de 88 px : il chevauchait la barre.');
console.log('\n  A verifier sur telephone, cache vide :');
console.log('  1. Plus de balai sur « Logements », plus de trois points sur « Argent ».');
console.log('  2. La barre flotte, ses angles ne sont plus rognes.');
console.log('  3. La capsule active est bien centree sous son onglet, non coupee.');
console.log('  4. Le badge « 7 » de Messages est toujours la.');
console.log('  5. bhVerifStyle()  — icones_posees: 5, ilot: true.\n');
