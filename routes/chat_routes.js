...
function appendMessage(message) {
  ...
  if (message.id) messageDiv.dataset.messageId = String(message.id);
...
// Mise à jour statut (✓ / ✓✓) en live
socket.on('message_status', ({ message_id, delivered_at, read_at, is_read }) => {
  ...
  const read = !!read_at || !!is_read;
  statusEl.textContent = read ? '✓✓' : (delivered ? '✓' : 'Envoyé');
});
